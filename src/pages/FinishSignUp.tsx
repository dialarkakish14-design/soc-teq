...
export function FinishSignUp({ email, onDone }: { email: string; onDone: () => void }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    supabase.auth.updateUser({ data: { full_name: fullName.trim(), username: username.trim().toLowerCase() } });
    onDone();
  }
  ...
  return (
    <div className="mx-auto min-h-dvh max-w-md px-5 py-8">
      ...
      <form onSubmit={handleSubmit} className="mt-6 flex flex-col gap-4">
        ...
        <button type="button" onClick={() => supabase.auth.signOut()} className="mt-1 text-sm font-semibold text-[#3F4C50]">
          Log out
        </button>
      </form>
    </div>
