...
-- Every PGY year's current-cycle data in one call, for the PD dashboard --
-- mirrors get_cycle_dashboard's shape closely but widened from "my own
-- PGY" to "every PGY in my program." No resident_id is ever paired with a
-- name here, only resident_code, same anonymity convention the resident-
-- facing app already uses cohort-wide.
...
-- Phase 4: "Impact evaluation" — the follow-up assessment against
-- baseline. Explicitly started, same as phases 2 and 3, rather than
-- silently appearing once 6 months are up: date-gated on 180 days so it
-- can't start early, but still requires a resident's click.
...
-- ---------- topic requests ----------
-- "What I'd want covered" on a claimed topic (Phase 3): a free-text
-- comment, one or more preset tags, or both — from any resident in the
-- cohort, not just whoever claimed it. See patch_topic_requests.sql.
